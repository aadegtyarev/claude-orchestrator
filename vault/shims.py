...
# Глобальные флаги git, чьё значение идёт ОТДЕЛЬНЫМ аргументом (`git --git-dir
# <dir> fetch`). Такой аргумент — не подкоманда, и шим обязан проглотить его
# вместе с флагом: иначе примет значение за подкоманду, не найдёт в GIT_NETWORK
# и уведёт сетевой вызов мимо кошелька, в локальный git без кредов хоста (живой
# регресс на `-C`, см. git_shim). Форма `--flag=value` одним аргументом сюда не
# относится — её съедает общая ветка `-*`.
# `--config` тут намеренно нет: глобального `git --config` не существует
# (только `git clone --config key=value`, а он идёт ПОСЛЕ подкоманды, которую
# шим уже нашёл). В guard'е кошелька он разбирается — там смотрят всю строку.
_VALUE_FLAGS = (
...
def git_shim(real_git: str | None = None) -> str:
    ...
    связано с кошельком/keyring — искали не там, см. инцидент).

    Пропускаем вместе со значением ВСЕ глобальные флаги git, берущие его
    ОТДЕЛЬНЫМ аргументом (_VALUE_FLAGS), а не только `-C`/`-c`: у `--git-dir`,
    `--work-tree`, `--namespace`, `--attr-source`, `--shallow-file`,
    `--super-prefix`, `--config-env` ровно та же форма, и `git --git-dir <dir>
    fetch` уходил бы мимо кошелька тем же путём, каким когда-то уходил `-C`.
    Форма `--flag=value` одним аргументом сюда не относится — её съедает общая
    ветка `-*`. argv для итогового exec не меняем — форвардим как пришло."""
    ...
    return (
        ...
        "# Подкоманда — первый аргумент без ведущего `-`; глобальные флаги,\n"
        "# берущие значение ОТДЕЛЬНЫМ аргументом, пропускаем вместе с ним —\n"
        "# иначе `git -C <dir> push` не находил бы push и молча уходил мимо\n"
        "# кошелька в локальный git без кредов хоста.\n"
        'sub="" skip=0\n'
        'for arg in "$@"; do\n'
        '  if [ "$skip" = 1 ]; then skip=0; continue; fi\n'
        '  case "$arg" in\n'
        f"    {value_flags}) skip=1 ;;\n"
        "    -*) ;;\n"
        '    *) sub="$arg"; break ;;\n'
        "  esac\n"
        "done\n"
