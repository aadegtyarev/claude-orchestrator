...
class Transport(Protocol):
    ...
    def known_origin(self, session: "Session", token: str) -> bool:
        """Мог ли ЭТОТ адаптер выдать такой токен для ЭТОЙ сессии.

        Проверка адресата ответа: токен непрозрачен для ядра, но сам адаптер
        знает его структуру и свои привязки. Telegram сверяет чат и топик, веб
        адресуется по имени сессии и пропускает всё.

        Нужна потому, что context_id приезжает от модели строкой: та может его
        выдумать (наблюдался живой случай — сессия «отвечала» в несуществующий
        личный чат, а доставка всё равно шла в привязанный топик, и модель
        считала, что говорит не туда). Неизвестный токен — не «доставить
        куда-нибудь», а явный отказ модели.

        Метод НЕОБЯЗАТЕЛЕН: ядро вызывает его через getattr, адаптер без него
        считается непроверяющим.
        """
        ...
