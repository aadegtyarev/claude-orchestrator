...
MESSAGES: dict[str, dict[str, str]] = {
    "ru": {
        ...
        "bg_live_n": "Живые процессы сессии ({n}) — прямо сейчас, из /proc:",
        "bg_stale": "⚠️ Снимку {age}: он обновляется только по концу хода, а ход идёт. Смотри живые процессы ниже.",
        "bg_never": "⚠️ Снимка ещё не было (ход ни разу не завершался). Смотри живые процессы ниже.",
        ...
        "session_limit": "⛔ Исчерпан лимит сессии подписки (5-часовое окно). Claude Code сам повторит запрос в {reset} — до этого ход стоит на месте, и новые сообщения модель не увидит. Это НЕ зависание: ждать или сменить бэкенд/модель (/model). Расход по лимитам — /usage.",
        "session_limit_noreset": "⛔ Исчерпан лимит сессии подписки (5-часовое окно). Claude Code ретраит сам, но до сброса ход стоит и новые сообщения модель не увидит. Это НЕ зависание. Расход по лимитам — /usage.",
        ...
        "perm_request_fallback": "🔐 <b>Запрос разрешения</b>: {tool}\nТекст слишком длинный для чата — детали в /log. Ответь кнопкой ниже.",
    ...
    "en": {
        ...
        "bg_live_n": "Live session processes ({n}) — right now, from /proc:",
        "bg_stale": "⚠️ Snapshot is {age} old: it only updates when a turn ends, and the turn is still running. See live processes below.",
        "bg_never": "⚠️ No snapshot yet (no turn has ended). See live processes below.",
        ...
        "session_limit": "⛔ Subscription session limit reached (5-hour window). Claude Code will retry by itself at {reset} — until then the turn is stuck and the model won't see new messages. This is NOT a hang: wait or switch backend/model (/model). Limit usage — /usage.",
        "session_limit_noreset": "⛔ Subscription session limit reached (5-hour window). Claude Code retries by itself, but until the reset the turn is stuck and the model won't see new messages. This is NOT a hang. Limit usage — /usage.",
        ...
        "perm_request_fallback": "🔐 <b>Permission request</b>: {tool}\nText too long for chat — details in /log. Reply with the button below.",
        "perm_allow": "✅ Allow",
        "perm_deny": "❌ Deny",
