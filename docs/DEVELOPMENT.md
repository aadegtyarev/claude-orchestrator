...
- **Разметка: один текст — один рендер.** Тексты ЯДРА (`core/texts.py` и
  ...
  Тексты МОДЕЛИ — markdown, их рендерит `core/mdrender.md_to_html`. Адаптер
  выбирает путь явно: Telegram — `_reply`/`_answer`/`_edit` и `_send(core=True)`
  для своих текстов, обычный `_send` для ответов модели; веб отдаёт текст ядра
  в поле `html` как есть, а `core.plain()` даёт его же без разметки. Второй
  проход рендера экранирует собственные теги текста — оператор увидит «<code>»
  глазами (так и случилось с `/info` и `/profile`). Инварианты сторожит
  `tests/textrender_test.py`; значения из внешнего мира (хвост лога, имя
  сессии, текст ошибки) экранируй `html.escape` в месте подстановки.
