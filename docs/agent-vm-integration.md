...
### Конфигурация сессии в гостя: что доезжает, а что нет (Шаг 4, 2026-07-21)

**Корень проблемы:** env процесса `claude` в гостя НЕ попадает (замерено: в
госте `ANTHROPIC_BASE_URL=unset`, `CLAUDE_CONFIG_DIR=unset`). Значит под
agent-vm молча пропадали `CLAUDE_ENV_*` — оператор задал свой прокси и был
уверен, что сессия ходит через него, а она шла мимо.

**Что доезжает и как:**

| Канал | Через границу VM |
|---|---|
| Флаги `claude` (`--model`, `--resume`, `--settings`, `--mcp-config`, …) | ✅ argv передаётся verbatim |
| Файлы (`.mcp.json`, `settings.local.json`, хук-скрипт, папки сессии/проекта) | ✅ `--mount` |
| `CLAUDE_ENV_*` | ✅ **через блок `env` в `settings.local.json`** — его читает САМ клиент (проверено: запросы уходят на заданный адрес) |
| env процесса | ❌ не течёт вовсе |
| `CLAUDE_CONFIG_DIR` | ❌ и не может: это хостовый путь с кредами; монтировать его в гостя = занести хостовые креды в VM. WARNING на старте |

**Свой прокси (`ANTHROPIC_BASE_URL`) под agent-vm РАБОТАЕТ** — но нужны три
вещи вместе, и каждая выяснена экспериментом:

1. **LAN-адрес хоста, а не loopback и не `host.microsandbox.internal`.**
   Внутри VM `127.0.0.1` — это сам гость. Гейтвей-имя agent-vm не
   маршрутизирует (запрос не доходит), а вот LAN-адрес хоста его прокси
   обходит — он сам кладёт его в `no_proxy` гостя. Config переписывает адрес
   автоматически (`host_lan_ip`: `src` дефолтного маршрута с наименьшей
   метрикой — ровно тот адрес, что выбирает сам agent-vm; трюк «UDP на 8.8.8.8»
   не годится, при VPN он вернёт адрес туннеля). Переопределяется
   `AGENT_VM_HOST_IP`.
2. **`--allow-egress <адрес хоста>`** — политика гостя `public_only` запрещает
   RFC1918. Раннер добавляет флаг сам и ровно под этот адрес (не `--allow-lan`:
   тот открыл бы гостю всю подсеть).
3. **Свой токен** (`CLAUDE_ENV_ANTHROPIC_AUTH_TOKEN` или `…_API_KEY`). В госте
   у claude своих кред НЕТ — их подставляет прокси agent-vm на проводе и только
   для СВОЕГО эндпоинта. При своём `base_url` подстановки нет, и claude падает
   «Execution error» ещё до запроса. С токеном тот же путь работает — проверено:
   запросы дошли до хостового сервиса (`/v1/messages?beta=true`). Без токена
   раннер отказывает на старте с объяснением, а не отдаёт нерабочие сессии.

⚠️ Побочный эффект токена: claude предупреждает, что коннекторы claude.ai
отключаются, когда задан внешний auth-источник.

⚠️ **Токен виден МОДЕЛИ.** Он попадает в `settings.local.json` сессии, а в госте
модель — root и читает свой каталог. Это не регресс: под bwrap `CLAUDE_ENV_*`
лежат в env процесса, откуда модель их тоже прочитает (`echo $VAR`) — именно
поэтому для секретов, которые модель видеть НЕ должна, существует кошелёк. Файл
пишется `0600` (защита от других пользователей хоста, не от модели). Вывод:
сюда кладут токен своего прокси, а не боевой ключ.

**IPv4-only:** `host_lan_ip` разбирает только IPv4-маршруты. На хосте с
IPv6-only дефолтом адрес не определится — переменная останется как есть, будет
WARNING; задавай `AGENT_VM_HOST_IP` вручную.
...
это уже ~10 GiB RAM при пяти запущенных сессиях; выставляй лимит с оглядкой на
хост. Страховка есть и без правильного лимита: перед стартом оркестратор
сверяет `MemAvailable` с ценой VM и порогом `MIN_FREE_RAM_MB` и отказывает
заранее — под `agent-vm` цена известна точно, это и есть `AGENT_VM_MEMORY_GIB`.
