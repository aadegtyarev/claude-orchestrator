...
| `SANDBOX_BWRAP_WALLET` | `true` при `bwrap` | Кошелёк секретов вкл/выкл. Сам объявляет модуль — `MODULES` трогать не нужно. Вне `SANDBOX=bwrap` игнорируется (с WARNING), т.к. кошелёк там не работает |
| `MODULES` | пусто | Реестр модулей поверх ядра. Кошелёк включается настройкой выше; legacy `MODULES=wallet` ещё работает, при конфликте побеждает `SANDBOX_BWRAP_WALLET` |
...
`SANDBOX` в `.env` — движок изоляции **по умолчанию**. Отдельной сессии его
можно переопределить при создании флагом `--box`:

```
/new тесты --box off      # сессия БЕЗ изоляции: полный доступ к хосту
/new бэкенд /home/u/api   # как обычно — движок по умолчанию
```

Выбор живёт с сессией (пишется в `.sessions.json`): `resume`/`clear` поднимут её
тем же движком. Что даёт эта сессия, следует её движку, а не `.env`: раннер,
`/bash` в топике, docker-прокси и кошелёк секретов. Доступные значения — `off`
и движок из `SANDBOX`; `vm` (agent-vm) выбирается только когда он и есть
`SANDBOX` (его обвязка — LAN-адрес хоста для гостя, guest-адреса, транскрипт
внутри VM — настраивается один раз на старте). Сессия с нестандартной изоляцией
помечена в `/list` и в веб-списке.

⚠️ **`--box off` и кошелёк.** Кошелёк секретов работает только под `bwrap`, и в
сессии без песочницы он не просто выключен: процесс идёт под тем же
пользователем ОС, поэтому модель там читает и сам `secrets.toml`, и провижн
соседних сессий (`.homes/<имя>/.wallet.json`). Флаг это разрешает — решение
оператора, — но `/new` сразу отвечает предупреждением, а в лог уходит WARNING.
Нужна изоляция секретов — не открывай сессию флагом `off`.
...
## Кошелёк секретов (`SANDBOX_BWRAP_WALLET`)

Включён по умолчанию при `SANDBOX=bwrap`; выключить — `SANDBOX_BWRAP_WALLET=false`.
Настройка сама объявляет модуль, `MODULES` для этого не нужен.

> **Требует `SANDBOX=bwrap`.** Кошелёк подключается к сессии через окружение
> процесса `claude` (обёртки первыми в PATH + env-маркеры секретов), а это
> работает, только когда `claude` — процесс на хосте. Под `SANDBOX=agent-vm`
> он живёт в microVM, куда наш env не попадает, поэтому кошелёк там **не
> включается даже при явном `SANDBOX_BWRAP_WALLET=true`** (иначе был бы «включён и молча
> ничего не делает» — худший вариант: оператор считает секреты защищёнными).
> В логе при старте будет явный WARNING.
>
> Под `agent-vm` креды **git/gh и самого Claude** держит его собственный прокси
> — с той же гарантией (в гость уходят плейсхолдеры, подмена на проводе), так
> что типовая работа с git/gh не страдает. Всё остальное — ssh/scp, произвольные
> токены для curl и любых CLI, inject-секреты, вымарывание значений в выводе,
> policy per-session/per-command — в VM-режиме недоступно; нужна такая сессия —
> запускай её под `bwrap`. Подробности и таблица покрытия —
> [docs/agent-vm-integration.md](docs/agent-vm-integration.md).
...
- при активном кошельке (`SANDBOX=bwrap`) оркестратор дополнительно
...
`/new [имя] /путь` (плюс `--box off` — сессия без изоляции, см. выше), `/list`, `/ls [путь]`, `/wallet` (policy кошелька, см. ниже),
...
`/close_session`, `/delete_session`, `/bash <cmd>` и `/bashin <ввод>`, `/bg`
(фоновые процессы/кроны сессии — что модель оставила крутиться);
...
чате** — на ХОСТЕ вне песочницы, полный доступ, старт из дома пользователя (для
`systemctl`, управления хостом) — как `/ls` и `/new`. Команда только оператора
(ALLOWED_USER_IDS), не модели.
